...
namespace VacaBook.Web.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult GeneratePPTExport(int id)
        {
            ...
            shape = slide.Shapes.FirstOrDefault(x => x.ShapeName == "txtVillaName") as IShape;
            if (shape is not null)
            {
                shape.TextBody.Text = villa.Name;
            }
            ...
            if (shape is not null)
            {
                shape.TextBody.Text = string.Format("{0}/Night", villa.Price.ToString("c"));
            }
            ...
            if (shape is not null)
            {
                ...
                {
                    imageUrl = string.Format("{0}{1}", basePath, "/img/placeholder.png");
                    imageData = System.IO.File.ReadAllBytes(imageUrl);
                }
